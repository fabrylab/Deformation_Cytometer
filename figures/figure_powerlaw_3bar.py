...
def plot_joint_density(x, y, label=None, only_kde=False, color=None, growx=1, growy=1, offsetx=0):
    ...
    c = colors.to_rgba("C2")
    c0 = np.array(c)*0.5+0.5*np.ones(4)
    cmap = pylustrator.LabColormap((c0, c), 255)
    cmap._init()
    plotDensityScatter(x, y, cmap=cmap, s=1)
...
for index, pressure in enumerate([3]):
    ...
    plt.plot([1e-1, 1e0, 1e1, 3e1], fit([1e-1, 1e0, 1e1, 3e1], *res.x)[0], "C0-", lw=0.8)
    plt.plot([1e-1, 1e0, 1e1, 3e1], fit([1e-1, 1e0, 1e1, 3e1], *res.x)[0], "k-", lw=0.8, alpha=0.3)
    plt.loglog(data.omega, data.Gp2, "o", alpha=0.25, label="G''", ms=1)
    plt.plot([1e-1, 1e0, 1e1, 3e1], fit([1e-1, 1e0, 1e1, 3e1], *res.x)[1], "C1-", lw=0.8)
    plt.plot([1e-1, 1e0, 1e1, 3e1], fit([1e-1, 1e0, 1e1, 3e1], *res.x)[1], "k-", lw=0.8, alpha=0.3)
    plt.ylabel("G' / G''")
...
plt.figure(1).axes[1].set_xticklabels(["$10^1$", "$10^2$", "$10^3$"], fontsize=9, fontweight="normal", color="black", fontstyle="normal", fontname="Arial", horizontalalignment="center")
