...
from deformationcytometer.evaluation.helper_functions import plot_velocity_fit, plot_density_hist, \
    plotDensityLevels, plotBinnedData, plot_joint_density, split_axes, get_mode_stats, bootstrap_match_hist
import numpy as np
...
for row_index, pressure in enumerate([0.5, 1, 2, 3]):
    ...
    for index, paths in enumerate(file_pairs):
        data0, config0 = load_all_data(paths[0], solidity_threshold=0.7, irregularity_threshold=1.3, pressure=pressure)
        data1, config1 = load_all_data(paths[1], solidity_threshold=0.7, irregularity_threshold=1.3, pressure=pressure)
        ...
        for i in range(10):
            ...
            stat_k0.append(get_mode_stats(data0b.k_cell))
            stat_k1.append(get_mode_stats(data1b.k_cell))
        stat_k0 = np.array(stat_k0)
        stat_k1 = np.array(stat_k1)
        ...
        try:
            ...
            #plt.errorbar(index, np.mean(stat_k1[:, 0]), np.std(stat_k1[:, 0]))
            plt.plot(index, np.mean(stat_k0[:, 0])/np.mean(stat_k1[:, 0]), "o")
...
plt.figure(1).axes[0].set_xlim(-0.25, 5.25)
plt.figure(1).axes[0].set_ylim(0.0, 1.2818077551966356)
plt.figure(1).axes[0].set_xticks([0.0, 2.0, 4.0])
plt.figure(1).axes[0].set_xticklabels(["1", "10", "100"], fontsize=10.0, fontweight="normal", color="black", fontstyle="normal", fontname="Arial", horizontalalignment="center")
...
plt.figure(1).axes[1].set_xlim(-0.25, 5.25)
plt.figure(1).axes[1].set_ylim(0.0, 1.2818077551966356)
plt.figure(1).axes[1].set_xticks([0.0, 2.0, 4.0])
plt.figure(1).axes[1].set_xticklabels(["1", "10", "100"], fontsize=10.0, fontweight="normal", color="black", fontstyle="normal", fontname="Arial", horizontalalignment="center")
...
plt.figure(1).axes[2].set_xlim(-0.25, 5.25)
plt.figure(1).axes[2].set_ylim(0.0, 1.2818077551966356)
plt.figure(1).axes[2].set_xticks([0.0, 2.0, 4.0])
plt.figure(1).axes[2].set_xticklabels(["1", "10", "100"], fontsize=10.0, fontweight="normal", color="black", fontstyle="normal", fontname="Arial", horizontalalignment="center")
...
plt.figure(1).axes[3].set_xlim(-0.25, 5.25)
plt.figure(1).axes[3].set_ylim(0.0, 1.2818077551966356)
plt.figure(1).axes[3].set_xticks([0.0, 2.0, 4.0])
plt.figure(1).axes[3].set_yticks([0.0, 0.25, 0.5, 0.75, 1.0, 1.25])
plt.figure(1).axes[3].set_xticklabels(["1", "10", "100"], fontsize=10.0, fontweight="normal", color="black", fontstyle="normal", fontname="Arial", horizontalalignment="center")
