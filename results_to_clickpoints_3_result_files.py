...
def addEllipses2(data_file2, db,video_file):
    ...
    type_elli_good = db.setMarkerType('GT', color='#0affe0', mode=db.TYPE_Ellipse)
    for line in data:
        frame_number = int(line[0])
...
def addEllipses3(data_file3, db,video_file):

    #Frame	x_pos	y_pos	RadialPos	LongAxis	ShortAxis	Angle	irregularity	solidity	sharpness
    #Pathname
    data = np.genfromtxt(data_file3, dtype=float, skip_header=2)    
...
data_file2 = video_file.replace(".avi", "_result.txt").replace(".tif", "_result_GT_new.txt")
data_file3 = video_file.replace(".avi", "_result.txt").replace(".tif", "_result_NN_new.txt")


cdb_file = video_file.replace(".avi", ".cdb").replace(".tif", "_all_ellipses.cdb")
...
addEllipses3(data_file3, db,video_file)
