...
display = 1 #set to 1 if you want to see every frame of im, set to 2 if you want to see im2, im3, im4
pixel_size = 0.37e-6 # in m for 20x AlliedVision 
...
#                ax4.imshow(im4,cmap='jet', vmin = 0, vmax = 3000 )                       
            ...
            for region in regionprops(label_image,im):
                if region.area >= 100 : #analyze only large, dark ellipses
                    l = region.label
                    structure = np.mean(im4[label_image==l])
...
#%% compute radial inetensity profile for each ellipse
 ...
 #%% select the "good" cells
                    if np.sqrt(structure)/im_mean > 0.09 and np.sqrt(structure)/im_mean <0.26 and region.mean_intensity/im_mean <0.99 and region.perimeter/circum<1.06 and region.area>500:
...
result_file = output_path + '/' + filename_base + '_struct_result.txt'
