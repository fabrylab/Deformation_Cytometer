...
for image_index, im in enumerate(vidcap):
    if len(im.shape) == 3:
        im = im[:,:,0]
    if count == 0:
        kymo = np.zeros((len(vidcap),im.shape[0]))
    print('\r',count, ' of ',len(vidcap), ' with ', len(frame), ' good cells         ', end='')
    ...
    for region in regionprops(label_imageo, im, coordinates='rc'): # region props are based on the original image, row-column style (first y, then x)
        ...
        if region.area >= Amin_pixels:# and np.sum(im3[region.slice])>Amin_pixels: #analyze only regions larger than 100 pixels,
            ...
            for d in range(0, int(3*r)):
                ...
                # rotate the points by the angle of the ellipse
                t = ellipse_angle
                xrot = (x *np.cos(t) - y*np.sin(t) + region.centroid[1]).astype(int)
                yrot = (x *np.sin(t) + y*np.cos(t) + region.centroid[0]).astype(int)                    
                # crop for points inside the image
                index = (xrot<0)|(xrot>=im.shape[1])|(yrot<0)|(yrot>=im.shape[0])                        
                x = xrot[~index]
                y = yrot[~index]
                if d == int(r_min):
                    features = im[y,x]-np.min( im[y,x])
                    features = features / np.mean(features)
                    kymo[count,int(region.centroid[0])-8:int(region.centroid[0])+8] = features
...
kymograph_file = output_path + '/' + filename_base + '_kymograph.tif'
kymo = 256*kymo/np.max(kymo)
imageio.imsave(kymograph_file, kymo.astype('uint8'))